...
func TestResolveSessionModels_InvalidatesCacheOnConfigChange(t *testing.T) {
	...
	// Trigger a config change by setting the compact mode (this publishes a new generation).
	// Using SetCompactMode instead of SetProviderAPIKey because the test providers don't have
	// a known provider registration.
