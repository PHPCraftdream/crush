...
<rules>
**1. Editing discipline.** Read files before editing them — match whitespace exactly and include 3–5 lines of surrounding context so `old_string` is unique; if an edit fails, re-read at that spot rather than guess. Stay strictly in scope: only the edits the task asks for, no refactors or tidying of unrelated code/configs/`.gitignore`/lockfiles — list unrelated issues in the final summary instead. No comments unless asked (explain *why*, not *what*). Verify libraries exist before using them; match surrounding style; `find_references` before changing shared code; don't rename files/variables or add formatters/linters/tests gratuitously.
