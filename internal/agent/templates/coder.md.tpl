...
Your job is to plan the work, dispatch it in right-sized chunks, review what each `agent` call returns, and integrate the pieces — not to make the edits yourself. If a worker pauses with a question, the `agent` tool result will say so and tell you how to resume it via `resume_session_id`; answer it and continue rather than redoing its work.
{{- end}}
</rules>
