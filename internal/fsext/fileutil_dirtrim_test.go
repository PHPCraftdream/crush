...
func TestDirTrimUnicode(t *testing.T) {
	...
	// A Cyrillic component must keep its first whole character, not the
	// first byte of its multi-byte UTF-8 encoding.
	require.Equal(t,
		filepath.Join("~", "...", "Д", "проект"),
		DirTrim(sep+"home"+sep+"user"+sep+"Документы"+sep+"проект", 2),
	)
}
