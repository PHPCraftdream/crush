...
// The whole check-then-compute-then-store sequence runs under a single write
// lock, so it is atomic: two concurrent callers for the same missing key
// cannot both observe a miss and both execute fn — the second caller blocks
// on the lock and, once it acquires it, sees the value the first caller just
// stored and returns that instead of computing its own.
//
// fn must not call GetOrSet (or any other method that takes m.mu) on this
// SAME Map instance — doing so would deadlock, since the lock held for the
// duration of fn is not reentrant. Calling into a *different* Map (or
// unrelated work) from fn is fine.
